...
describe('ActorRdfMetadataExtractLinkFilterVoid', () => {
  ...
  beforeEach(() => {
    ...
    actor = new ActorRdfMetadataExtractLinkFilterVoid({
      bus,
      name: 'actor',
    });
  ...
  describe('run', () => {
    ...
    it('should add filter for uriSpace when sparqlEndpoint and uriSpace share same subject', async() => {
      ...
      const quads: RDF.Quad[] = [
        DF.quad(
          ...
          DF.namedNode(rdf_void.sparqlEndpoint),
          DF.literal('http://example.org/sparql'),
        ),
        DF.quad(
          DF.namedNode(datasetUri),
          DF.namedNode(rdf_void.uriSpace),
          DF.literal('http://example.org/data/'),
        ),
      ];
      const action = {
        url: 'http://example.org',
    ...
    it('should add filter for uriRegexPattern when sparqlEndpoint and uriRegexPattern share same subject', async() => {
      ...
      const quads: RDF.Quad[] = [
        ...
        DF.quad(
          DF.namedNode(datasetUri),
          DF.namedNode(rdf_void.uriRegexPattern),
          DF.literal('http://example.org/api/.*'),
        ),
      ];
      const action = {
        url: 'http://example.org',
    ...
    it('should not add filter when sparqlEndpoint is missing', async() => {
      ...
      const quads: RDF.Quad[] = [
        DF.quad(
          DF.namedNode('http://example.org/otherDataset'),
          DF.namedNode(rdf_void.uriSpace),
          DF.literal('http://example.org/data/'),
        ),
      ];
    ...
    it('should handle multiple datasets with different uriSpaces', async() => {
      ...
      const quads: RDF.Quad[] = [
        DF.quad(
          DF.namedNode('http://example.org/dataset1'),
          DF.namedNode(rdf_void.sparqlEndpoint),
          DF.literal('http://example.org/sparql1'),
        ),
        DF.quad(
          DF.namedNode('http://example.org/dataset1'),
          DF.namedNode(rdf_void.uriSpace),
          DF.literal('http://example.org/data1/'),
        ),
        DF.quad(
          DF.namedNode('http://example.org/dataset2'),
          DF.namedNode(rdf_void.sparqlEndpoint),
          DF.literal('http://example.org/sparql2'),
        ),
        DF.quad(
          DF.namedNode('http://example.org/dataset2'),
          DF.namedNode(rdf_void.uriSpace),
          DF.literal('http://example.org/data2/'),
        ),
      ];
      const action = {
        url: 'http://example.org',
