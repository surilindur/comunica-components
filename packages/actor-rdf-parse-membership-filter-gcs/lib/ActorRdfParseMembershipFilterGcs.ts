 ...
 */
export class ActorRdfParseMembershipFilterGcs extends ActorRdfParseMembershipFilter {
  public static readonly MEM: string = 'http://semweb.mmlab.be/ns/membership#';
  public static readonly MEM_FILTER: string = `${ActorRdfParseMembershipFilterGcs.MEM}filter`;
  ...
  public async test(action: IActionRdfParseMembershipFilter): Promise<IActorTest> {
    if (!action.data.some(quad => quad.predicate.value === ActorRdfParseMembershipFilterGcs.MEM_FILTER)) {
  ...
  public async run(action: IActionRdfParseMembershipFilter): Promise<IActorRdfParseMembershipFilterOutput> {
    const filter = Buffer.from(
      action.data.find(quad => quad.predicate.value === ActorRdfParseMembershipFilterGcs.MEM_FILTER)!.object.value,
      'base64',
    );
