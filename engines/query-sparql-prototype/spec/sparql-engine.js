...
const QueryEngine = require('@comunica/query-sparql-prototype').QueryEngine;
  ...
  return {
    ...
    async queryLdf(sources, proxyUrl, queryString, options) {
      ...
      if (result.resultType === 'quads') {
        return new RdfTestSuite.QueryResultQuads(await arrayifyStream.default(await result.execute()));
      }
      if (result.resultType === 'bindings') {
        return new RdfTestSuite.QueryResultBindings(
          (await result.metadata()).variables.map(variable => `?${variable.value}`),
          (await arrayifyStream.default(await result.execute()))
...
module.exports = createEngine(new QueryEngine());
